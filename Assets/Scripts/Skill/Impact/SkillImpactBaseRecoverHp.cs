...
public class SkillImpactBaseRecoverHp : ISkillImpact
{
    public IEnumerator DelorySkillImpact(SkillImpactData impactData, GridPosition targetPosition, Character owner)
    {
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            {
                int recoverHp = targetCharacter.Status.maxHp * impactData.ImpactDuration / 100 + impactData.ImpactValue;
                targetCharacter.RecoverHp(owner, recoverHp);
            }
        }
        yield return null;
    }
