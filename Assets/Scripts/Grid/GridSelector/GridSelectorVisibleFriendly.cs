...
public class GridSelectorVisibleFriendly : IGridSelector
{
    public List<GridPosition> GetValidGridPositions(GridPosition startPosition, int distance, E_Camp_Type campType)
    {
        ...
        for (int x = -distance; x <= distance; x++)
        {
            for (int y = -distance; y <= distance; y++)
            {
                ...
                if (!GridSystem.Instance.TryGetCharacterOnGridPosition(testGridPos, out Character character)) continue;
                if (character.Status.campType != campType) continue;
